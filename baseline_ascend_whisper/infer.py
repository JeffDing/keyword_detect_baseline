...
@torch.no_grad()
def predict(model, zip_path, csv_path, prefix, device):
...
def main():
    ...
    model = SiameseKWS(whisper_model, ckpt.get("embed_dim", TRAIN.embed_dim)).to(device)
    model.load_state_dict(ckpt["model"])
    model.eval()
    print(f"loaded {args.ckpt} (dev mean AUC={ckpt.get('auc')})")

    rows = predict(model, PATHS.eval_seen_zip, PATHS.eval_seen_csv, "seen", device)
    rows += predict(model, PATHS.eval_unseen_zip, PATHS.eval_unseen_csv, "unseen", device)
