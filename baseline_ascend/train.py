...
def parse_args():
    ...
    ap.add_argument("--cache-dir", type=str, default=PATHS.cache_dir,
                    help="mel 特征缓存目录，留空则不用缓存")
...
def main():
    ...
    train_ds = PairDataset(train_pairs, PATHS.train_zip, AUDIO, cache_dir=cache_dir)
    train_loader = DataLoader(train_ds, batch_size=args.bs, shuffle=True,
                              num_workers=args.workers, collate_fn=collate,
                              drop_last=True)

    def dev_loader(zip_p, csv_p):
        ds = PairDataset(load_pairs(csv_p, True), zip_p, AUDIO, cache_dir=cache_dir)
