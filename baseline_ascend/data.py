...
def _cache_path(cache_dir: str, zip_path: str, name: str) -> str:
    key = f"{zip_path}:{name}"
    digest = hashlib.sha256(key.encode("utf-8")).hexdigest()[:16]
    return os.path.join(cache_dir, f"{digest}.npy")


def _load_cached_feat(cache_dir: str, zip_path: str, name: str) -> Optional[torch.Tensor]:
    path = _cache_path(cache_dir, zip_path, name)
...
def _save_cached_feat(cache_dir: str, zip_path: str, name: str, tensor: torch.Tensor) -> None:
    if not cache_dir:
        return
...
class PairDataset(Dataset):
    def __init__(self, pairs: List[dict], zip_path: str, cfg: AudioConfig,
                 inference: bool = False, cache_dir: Optional[str] = None,
        ...
        self.save_cache = save_cache and self.cache_dir is not None
        self.logmel = LogMel(cfg)
        self._cache_index = set()
        if self.cache_dir is not None and os.path.isdir(self.cache_dir):
            self._cache_index = {
                name for name in os.listdir(self.cache_dir) if name.endswith(".npy")
            }
    ...
    def _feat(self, wav_name: str) -> torch.Tensor:
        if self.cache_dir is not None:
            digest = hashlib.sha256(f"{self.zip_path}:{wav_name}".encode("utf-8")).hexdigest()[:16]
            if f"{digest}.npy" in self._cache_index:
        ...
        tensor = pad_spec(spec, self.cfg.max_frames)

        if self.save_cache:
            _save_cached_feat(self.cache_dir, self.zip_path, wav_name, tensor)
